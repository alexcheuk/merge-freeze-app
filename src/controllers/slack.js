...
import {
  ...
  openUnfreezePRConfirmationDialog,
} from "../services/slack";
...
export const postMergeFreeze = async (req, res) => {
  try {
    ...
    await openConfirmationDialog(
      ...
      req.body.trigger_id,
      req.body.text,
      installedRepos.map((repo) => repo.name)
    );
...
export const postMergeUnfreezePR = async (req, res) => {
  try {
    ...
    if (installedRepos.length === 1) {
      ...
      await openUnfreezePRConfirmationDialog(
        installation.slackBotToken,
        req.body.trigger_id,
        prID,
        installedRepos
...
export const postMergeFreezeStats = async (req, res) => {
  ...
  try {
    ...
    const allTimeStatus = await MergeFreezeStatus.getStatusListByRepo(
      owner,
      repo
    );
    // Not optimized, regrabbing subsets from data.
    const thisMonthStatus = await MergeFreezeStatus.getStatusListByRepo(
      owner,
      repo,
      {
        datetime: {
    ...
    const lastMonthTimeStatus = await MergeFreezeStatus.getStatusListByRepo(
      owner,
      repo,
      {
        datetime: {
    ...
    return res.json(
      generateStatsMessage(allTimeStats, thisMonthStats, lastMonthStatus)
    );
  } catch (e) {
    console.log(e);
    res.send("Pull Request does not exist.");
